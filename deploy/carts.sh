...
echo "Finished rsync of Carts Service files."

echo "Starting setting Carts service environment file to dev..."
ssh -v -i ~/.ssh/amazon.pem ec2-user@searchproducts.alex-lindsay.com cp /var/www/api/carts/.env.dev /var/www/api/carts/.env
echo "Finished setting Carts service environment file to dev."
